...
void CesiumGDConfig::set_access_token(const String& accessToken)
{
	...
	Ref<FileAccess> file = open_file_access_with_err(String(CACHE_PATH) + "/ion_session.dat", FileAccess::ModeFlags::WRITE, &err);
	if (err != Error::OK) {
		ERR_PRINT(errMsg + REFLECT_ERR_NAME(err));
	}
...
void CesiumGDConfig::_enter_tree() {
	...
	Error err;
	Ref<FileAccess> file = open_file_access_with_err(String(CACHE_PATH) + "/ion_asset.dat", FileAccess::ModeFlags::WRITE_READ, &err);
	this->set_access_token(file->get_as_text());
}
