...
func _parse_property(object, type, name, hint_type, hint_string, usage, wide):
	if parse_tileset(object, type, name, hint_type, hint_string, usage, wide):
		return true
	return false

func parse_tileset(object, type, name, hint_type, hint_string, usage, wide):
	if !object.is_class("CesiumGDTileset"):
		return false
	if (name == "maximum_screen_space_error"):
